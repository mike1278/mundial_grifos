<template>
    <div>
        <div class="flex justify-between mb-4 px-2">
            <h2 class="text-gray-700 text-xl">Colores</h2>
            <link-button is="link" :href="route('colors.create')" variant="primary">Crear</link-button>
        </div>
        <c-table>
            <template #header>
                <tr>
            ...
            <tr v-for="data in colors.data" :key="data.id">
        ...
        <paginator :paginator="colors"></paginator>
        <modal :show="trash" v-if="rowTrash">
            <div class="flex justify-between border-b border-gray-200 border-solid">
                <h3 class="text-gray-700 text-lg py-2 px-2">
                    Desea eliminar el color '{{ rowTrash.name }}'
                </h3>
                <button
...
export default {
    ...
    methods: {
        ...
        trash(){
            Inertia.delete(this.route('colors.destroy',this.rowTrash.id), {
                preserveScroll: true,
                onSuccess: () => {
